function grandTourPuzzleToString(p_pathArray){
	...
	for(var iy = 0;iy < yLength;iy++)
		for(var ix = 0;ix < xLength;ix++){
			if (p_pathArray[iy][ix].state == CLOSED){
				gridChain+='X';
			}
			else{
				valueSpace=0;
				if (p_pathArray[iy][ix].wallR == PATH.YES){
					valueSpace+=1;
				}
				if (p_pathArray[iy][ix].wallD == PATH.YES){
					valueSpace+=2;
				}
...
*/
function stringToGrandTourPuzzle(p_string){
...
function charToSpace(p_char){
	switch(p_char){
		case('0'): return {state:OPEN,wallD:PATH.NO,wallR:PATH.NO};break;
		case('1'): return {state:OPEN,wallD:PATH.NO,wallR:PATH.YES};break;
		case('2'): return {state:OPEN,wallD:PATH.YES,wallR:PATH.NO};break;
		case('3'): return {state:OPEN,wallD:PATH.YES,wallR:PATH.YES};break;
		default : return  {state:CLOSED,wallD:PATH.NO,wallR:PATH.NO};break;
	}
}
