...
Drawer.prototype.drawSolverLinkInsideSpaces = function (p_context, p_coloursSet, p_solver, p_wallGrid) {
            ...
            if (iy < ( p_solver.yLength-1)) {
				if (p_solver.getLinkDown(ix, iy) == LOOP_STATE.LINKED) {
					...
					if (p_wallGrid && p_wallGrid.getWallD(ix, iy)) {
						p_context.fillStyle = p_coloursSet.noLinkWall ? p_coloursSet.noLinkWall : p_coloursSet.noLink; // For region loop solvers
					} else {
						p_context.fillStyle = p_coloursSet.noLink;
					}
	...
	if (p_solver.ergonomicOptions.displayOppositeEnds) {
		...
		p_context.fillStyle = p_coloursSet.oppositeSpaceWrite;
		if (!p_coloursSet.oppositeSpaceWrite) {
