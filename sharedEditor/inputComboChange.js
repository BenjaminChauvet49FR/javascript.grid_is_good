...
function comboChange(p_docElement, p_canvas, p_drawer, p_editorCore, p_saveLoadMode, p_fields, p_buttons, p_analyzerModes) {
	...
	var saveLoadMode = null; // saveLoadMode and not saveLoadModeId ! Because it contains an "id" property.
	
	// Specific options
    switch (content) { // Should break be forgotten, following instructions are read... and saveLoadMode is overset ! This was problematic with Stitches !
		// Which is why 'break' is put on its own line.
		case 'CurvingRoad' : 
			inputOptions.monoSymbolToEnter = SYMBOL_ID.WHITE;
			inputOptions.monoSymbolGridId = GRID_ID.PEARL;
			usesChains = false;
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.CURVING_ROAD;
			p_editorCore.setVisibleGrids([GRID_ID.PEARL]);
			fillListModes(p_analyzerModes, [ANALYZER_MODE.NOT_GREY]); 
			break;
		case 'Masyu':
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.MASYU;
			p_editorCore.setVisibleGrids([GRID_ID.PEARL]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.PEARLS]); 
			break;
		case 'Fillomino': case 'Rukkuea': case 'SlitherLink': case 'Usotatami':
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.NUMBERS_ONLY;
			...
			fillListModes(p_analyzerModes, [ANALYZER_MODE.BLUE_ON_WHITE, ANALYZER_MODE.BLACK_ON_WHITE, ANALYZER_MODE.BLUE_ON_WHITE]); 
			break;
		case 'Aqre': case 'AYE-Heya': case 'Chocona': case 'CountryRoad': case 'Detour': case 'Heyawake': case 'Regionalin': case 'Shimaguni': case 'Stostone':
			saveLoadMode = PUZZLES_KIND.REGIONS_NUMERICAL_INDICATIONS;
			p_editorCore.setVisibleGrids([GRID_ID.NUMBER_REGION]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.GREY_NUMBERS_WALLED, ANALYZER_MODE.NUMBERS_LEFT_UP_WALLED]); 
			break;
		case 'Hakoiri':
			saveLoadMode = PUZZLES_KIND.REGIONS_PLAYSTATION_SHAPES;
			p_editorCore.setVisibleGrids([GRID_ID.PLAYSTATION_SHAPES]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.PLAYSTATION_SHAPES]); 
			break;
		case 'Hakyuu': case 'Usoone': 
			saveLoadMode = PUZZLES_KIND.REGIONS_NUMBERS;
			p_editorCore.setVisibleGrids([GRID_ID.NUMBER_SPACE]); 
			if (content == 'Usoone') {
				inputOptions.forceMonoCharacter = true;
				inputOptions.maxNumber = 4;
				fillListModes(p_analyzerModes, [ANALYZER_MODE.NOT_GREY_WALLED]); 
				break;
			} else {
		...
		case 'SternenSchlacht':
			usesChains = false;
			saveLoadMode = PUZZLES_KIND.STAR_BATTLE; 
			p_editorCore.maskAllGrids();
			hasStars = true; 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.WALLS]); 
			break;
		case 'Suraromu':
			saveLoadMode = PUZZLES_KIND.SURAROMU; 
			p_editorCore.maskAllGrids();
			p_editorCore.setWallsOff();
			p_editorCore.setVisibleGrids([GRID_ID.SURAROMU]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.SURAROMU]); 
			break;
		case 'Akari': case 'Koburin': case 'Linesweeper': case 'Shakashaka': case 'Shugaku': case 'Sukoro':
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.DIGITS_X_ONLY;
			inputOptions.maxNumber = (content == 'Linesweeper' ? 8 : 4);
			if (content == 'Sukoro') { // Note : I have yet to see a Sukoro puzzle with Xs but hey, that's a challenge !
		...
		case 'Raitonanba' :
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.SYMBOLS_IN_SQUARE;
			inputOptions.minNumber = 0;
			inputOptions.maxNumber = 2;
			p_editorCore.setVisibleGrids([GRID_ID.DIGIT_X_SPACE]);
			fillListModes(p_analyzerModes, [ANALYZER_MODE.BLUE_ON_WHITE, ANALYZER_MODE.BLACK_ON_WHITE, ANALYZER_MODE.BLUE_ON_WHITE]); 			
			break;
		case 'Firumatto' :
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.DIGITS_QUESTION_ONLY;
			inputOptions.minNumber = 1;
			inputOptions.maxNumber = 4;
			fillListModes(p_analyzerModes, [ANALYZER_MODE.BLUE_ON_WHITE]);
			p_editorCore.setVisibleGrids([GRID_ID.DIGIT_QUESTION_SPACE]); 
			break;
		case 'CanalView': case 'Corral': case 'Geradeweg': case 'Kuromasu': case 'Nurikabe':
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.NUMBERS_X_ONLY;
		...
		case 'Yajilin': case 'Yajikabe': // Can also include Yajisan-Kazusan
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.YAJILIN_LIKE;
			p_editorCore.setVisibleGrids([GRID_ID.YAJILIN_LIKE]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.YAJI]); 
			break;
		case 'CastleWall': 
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.YAJILIN_BLACK_WHITE;
			p_editorCore.setVisibleGrids([GRID_ID.YAJILIN_BLACK_WHITE]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.CASTLE_WALL]); 
			break;
		case 'Shingoki':
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.NUMBER_BLACK_WHITE;
			inputOptions.minNumber = 2;
			p_editorCore.setVisibleGrids([GRID_ID.NUMBER_BLACK_WHITE]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.NUMBER_BALL_BLACK_WHITE, ANALYZER_MODE.SHINGOKI_DOTTED]); 
			break;
		case 'Tapa': 
			p_editorCore.setWallsOff();
			saveLoadMode = PUZZLES_KIND.TAPA;
			p_editorCore.setVisibleGrids([GRID_ID.TAPA]); 
			fillListModes(p_analyzerModes, [ANALYZER_MODE.NOT_GREY, ANALYZER_MODE.FROM_BLACK_2]);
			break;
		case 'Stitches':
			usesChains = false;
			saveLoadMode = PUZZLES_KIND.STITCHES;
		...
		case 'Gappy':
			usesChains = false;
			saveLoadMode = PUZZLES_KIND.ONLY_ONE_NUMBER_LEFT_UP_SQUARE;
			p_editorCore.setMarginInfo(MARGIN_KIND.NUMBERS_LEFT_UP);
			p_editorCore.setWallsOff();
			break; 
		case 'Putteria':
			saveLoadMode = PUZZLES_KIND.XS_AND_ONE_O_PER_REGION;
			p_editorCore.setVisibleGrids([GRID_ID.OX]);
			fillListModes(p_analyzerModes, [ANALYZER_MODE.NUMBERS_WALLED]);
			break; 
		case 'Sudoku':
			saveLoadMode = PUZZLES_KIND.SUDOKU;
		...
		case 'Galaxies':
			usesChains = false;
			saveLoadMode = PUZZLES_KIND.GALAXIES;
		...
		case 'GrandTour':
			usesChains = false;
			saveLoadMode = PUZZLES_KIND.WALLS_ONLY;
	...
	copySaveModeInto(saveLoadMode, p_saveLoadMode);
	adaptCanvasAndGrid(p_canvas, p_drawer, p_editorCore);
	
	const squarePuzzle = correspondsToSquarePuzzle(saveLoadMode); 
	...
	maskElementConditional(p_fields.spanSelectSudoku, saveLoadMode != PUZZLES_KIND.SUDOKU);
