...
function clickSpaceAction(p_solver, p_spaceIndexX, p_spaceIndexY, p_action) {
	switch(p_action.id) {
		...
		break;	
		case ACTION_PASS_ALL_REGIONS_SIZE.id :
			p_solver.emitPassAllRegionsSize(p_spaceIndexX, p_spaceIndexY);
		break; 
	}
}
...
loadPuzzle = function(p_canvas, p_drawer, p_solver, p_loadedString) {
	const loadedItem = stringToXsAndOneOPerRegionPuzzle(p_loadedString);
