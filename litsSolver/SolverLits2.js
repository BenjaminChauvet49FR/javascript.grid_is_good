...
SolverLITS.prototype.deductionsEventsTetrominoIdentification = function(p_listEventsToApply, p_indexRegion) {
	...
	if (shape == LITS.UNDECIDED) {
		...
		p_listEventsToApply.push(new ShapeRegionEvent(p_indexRegion, shape));
	}
}

// When a region contains 3 open spaces
SolverLITS.prototype.deductionsEventsTripletPlacement = function(p_listEventsToApply, p_indexRegion) {
	const firstSpace = getFirstLexicalOrderSpace(this.regions[p_indexRegion].openSpaces);
	const x1 = firstSpace.x;
	const y1 = firstSpace.y;
	const beforeLength = p_listEventsToApply.length;
	var ok = false;
	if (this.isOpenInRegionAtRight(x1+1, y1, p_indexRegion)) { //10
		if (this.isOpenInRegionAtRight(x1+2, y1, p_indexRegion)) { 
			this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 1);
			ok = true;
		} else if (y1 <= this.yLength-2) {
			if (this.isOpenInRegion(x1+2, y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 11);
				ok = true;
			} else if (this.isOpenInRegion(x1+1, y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 4);
				ok = true;
			} else if (this.isOpenInRegion(x1, y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 3);
				ok = true;
			} else if (this.isOpenInRegionAtLeft(x1-1, y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 14);
				ok = true;
			}
		} 
	} else if (y1 <= this.yLength-2) {
		if (this.isOpenInRegion(x1, y1+1, p_indexRegion)) {  // 01
			if (this.isOpenInRegionAtRight(x1+1,y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 6);
				ok = true;
			} else if (this.isOpenInRegionAtDownRight(x1+1,y1+2, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 12);
				ok = true;
			} else if (this.isOpenInRegionAtDown(x1,y1+2, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 2);
				ok = true;
			} else if  (this.isOpenInRegionAtDownLeft(x1-1,y1+2, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 13);
				ok = true;
			} else if (this.isOpenInRegionAtLeft(x1-1,y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 5);
				ok = true;
			}
		} else if (this.isOpenInRegionAtRight(x1+1, y1+1, p_indexRegion)) { // 11
			if (this.isOpenInRegionAtRight(x1+2,y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 7);
				ok = true;
			} else if (this.isOpenInRegionAtDown(x1+1,y1+2, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 8);
				ok = true;
			}
		} else if (this.isOpenInRegionAtLeft(x1-1, y1+1, p_indexRegion)) { // -11
			if (this.isOpenInRegionAtDown(x1-1,y1+2,p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 9);
				ok = true;
			} else if (this.isOpenInRegionAtLeft(x1-2,y1+1, p_indexRegion)) {
				this.deductionsShapeFrom3Open(p_listEventsToApply, x1, y1, p_indexRegion, 10);
				ok = true;
			}
		}
	}
...
*/ 
SolverLITS.prototype.deductionsShapeFrom3Open = function(p_listEventsToApply, p_x, p_y, p_ir, p_identifiant) {
	const eventsByConfig = arrayOfEventsBy3SpaceConfig[p_identifiant];
	
	const startingEventsLength = p_listEventsToApply.length;
	var currentEventsLength = p_listEventsToApply.length;
	this.deductionsPushShapeEventsDelta(p_listEventsToApply, p_x, p_y, p_ir, eventsByConfig[0], LITS.L);
	const noLFound = (p_listEventsToApply.length == currentEventsLength);
	
	currentEventsLength = p_listEventsToApply.length;
	this.deductionsPushShapeEventsDelta(p_listEventsToApply, p_x, p_y, p_ir, eventsByConfig[1], LITS.I);
	const noIFound = (p_listEventsToApply.length == currentEventsLength);
	
	currentEventsLength = p_listEventsToApply.length;
	this.deductionsPushShapeEventsDelta(p_listEventsToApply, p_x, p_y, p_ir, eventsByConfig[2], LITS.T);
	const noTFound = (p_listEventsToApply.length == currentEventsLength);
	
	currentEventsLength = p_listEventsToApply.length;
	this.deductionsPushShapeEventsDelta(p_listEventsToApply, p_x, p_y, p_ir, eventsByConfig[3], LITS.S);
	const noSFound = (p_listEventsToApply.length == currentEventsLength);
	
	if (p_listEventsToApply.length == startingEventsLength + 1) {
		// Only one event ! That means only one possibility of 4th open space ! 
		const onlyShapeEvent = p_listEventsToApply[p_listEventsToApply.length-1];
		p_listEventsToApply.push(new SpaceEvent(onlyShapeEvent.x, onlyShapeEvent.y, ADJACENCY.YES));
		...
		if (noSFound) {
			...
			this.deductionsAffectShapes(p_listEventsToApply, p_x, p_y, p_identifiant, LITS.S);
		}
	} else if (noSFound) {
		if (noLFound) {
			if (noIFound) {
				this.deductionsAffectShapes(p_listEventsToApply, p_x, p_y, p_identifiant, LITS.T);
			} else if (noTFound) {
				this.deductionsAffectShapes(p_listEventsToApply, p_x, p_y, p_identifiant, LITS.I);
			}
		} else if (noIFound && noTFound) {
			this.deductionsAffectShapes(p_listEventsToApply, p_x, p_y, p_identifiant, LITS.L);
		}
	}
}
...
SolverLITS.prototype.deductionsPushShapeEventsDelta = function (p_listEventsToApply, p_x, p_y, p_ir, p_spaceDeltas, p_shape) {
	p_spaceDeltas.forEach(delta => {
		...
		if ((y >= 0) && (y < this.yLength) && (x >= 0) && (x < this.xLength) && (this.regionArray[y][x] == p_ir) && (this.answerArray[y][x] != ADJACENCY.NO)) {
			p_listEventsToApply.push(new ShapeEvent(x, y, p_shape));
		}
	});
}
...
function getFirstLexicalOrderSpace(p_spaceArray) {
	var result1stCoor = p_spaceArray[0];
	for (var i = 1; i < p_spaceArray.length ; i++) {
		if ((p_spaceArray[i].y < result1stCoor.y) || ((p_spaceArray[i].y == result1stCoor.y) && (p_spaceArray[i].x < result1stCoor.x))) {
			result1stCoor = p_spaceArray[i];
		}
	}
	return result1stCoor;
}
...
SolverLITS.prototype.deductionsAffectShapes = function(p_listEventsToApply, p_x, p_y, p_config, p_shape) {
	const spaceDeltas = arrayOfSpaceDeltasBy3SpaceConfig[p_config];
	p_listEventsToApply.push(new ShapeEvent(p_x, p_y, p_shape));
	p_listEventsToApply.push(new ShapeEvent(p_x+spaceDeltas[0].x, p_y+spaceDeltas[0].y, p_shape));
	p_listEventsToApply.push(new ShapeEvent(p_x+spaceDeltas[1].x, p_y+spaceDeltas[1].y, p_shape));
}
