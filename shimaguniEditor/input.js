...
clickSpaceAction = function(p_editorCore,p_x, p_y, p_modes){
	...
	if (mode.id == MODE_SELECTION.id){
		...
		var done = false; //TODO c'est vraiment pas pratique, de ne pas pouvoir faire un if... else (puisque la condition devient fausse dans les instructions du if)
		if (p_editorCore.getNumber(p_x,p_y) == 0){
